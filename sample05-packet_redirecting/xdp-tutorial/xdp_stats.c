		      ...
		      __u32 map_type, int interval)
{
	...
	while (1) {
		...
		} else if (id != info.id) { // BPF Map ID를 비교하는 로직
			//    함수 인자로 받아왔던 기존 ID(id)와 
			//    방금 파일을 열어서 확인한 ID(info.id)를 비교합니다.
...
int main(int argc, char **argv)
{
	...
	/* [수정됨] BPF 경로를 찾기 위해 인터페이스 이름의 '.'을 '_'로 변환 */
	char dir_name[IF_NAMESIZE];
	strncpy(dir_name, cfg.ifname, sizeof(dir_name));

	// 문자열을 순회하며 '.'이 있으면 '_'로 변경
	...
	// cfg.ifname 대신 변환된 dir_name을 사용
	len = snprintf(pin_dir, PATH_MAX, "%s/%s", pin_basedir, dir_name);
