...
int xdp_port_rewrite_func(struct xdp_md *ctx)
{
    ...
    /* 2. IP 헤더 파싱 (IPv4 및 IPv6 모두 처리) */
    int l4_proto = 0;

    if (nh_type == bpf_htons(ETH_P_IP)) {
        struct iphdr *iph;
        l4_proto = parse_iphdr(&nh, data_end, &iph);
    } else if (nh_type == bpf_htons(ETH_P_IPV6)) {
        struct ipv6hdr *ip6h;
        l4_proto = parse_ip6hdr(&nh, data_end, &ip6h);
    } else {
        /* IP 패킷이 아니면 그냥 통과 */
        return XDP_PASS;
    }

    /* 3. L4 헤더 파싱 및 포트 변조 (Assignment 핵심) */
