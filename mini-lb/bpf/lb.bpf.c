...
// 1. 설정 정보를 담을 구조체 (백엔드 서버 정보)
struct lb_config {
    __u32 real_server_ip; // 백엔드 서버의 실제 IP
    __u32 lb_vip;         // 로드밸런서의 가상 IP (Source IP로 사용)
    unsigned char src_mac[6]; // LB의 MAC 주소
    unsigned char dst_mac[6]; // Gateway 또는 백엔드 서버의 MAC 주소
};
